...
| gender         | ENUM         | NOT NULL, DEFAULT 'MALE'        | 성별 (MALE, FEMALE) - AI 이미지 생성 시 주인공 성별 결정                               |
| persona        | ENUM         | NOT NULL, DEFAULT 'BEST_FRIEND' | 페르소나 (BEST_FRIEND, PARENTS, EXPERT, MENTOR, COUNSELOR, POET) - 기본값: BEST_FRIEND |
...
- `gender`는 필수 입력 항목 (AI 이미지 생성 퀄리티를 위해 필수)
...
- 일기 수정 시 `image_url`은 수정된 내용(제목, 본문, 감정 등)을 반영하여 **재생성**됨
...
본 프로젝트는 User 테이블의 물리적 삭제(Hard Delete) 시, 연관된 데이터의 무결성을 위해 **Application-Level Cascade (Hibernate @OnDelete)** 방식을 사용합니다.

- **User 삭제 시 자동 삭제되는 테이블**:

  - diaries (일기) -> diary_images, diary_activities (일기 하위 데이터 포함)
  - refresh_tokens (리프레시 토큰)
  - risk_detection_sessions (위험 감지 세션)
  - error_logs (에러 로그)

- **별도 삭제 처리 (FK 미설정)**:
  - email_verification_codes: 재가입 시 Service 로직에서 삭제
  - password_reset_codes: 재가입 시 Service 로직에서 삭제
