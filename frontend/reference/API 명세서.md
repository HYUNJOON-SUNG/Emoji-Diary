...
- `persona`: 페르소나 (선택, 미제공 시 기본값 `"베프"`로 설정됨)
...
```json
{
  ...
  "weather": "맑음",
  "activities": ["운동", "독서"],
  "images": ["https://example.com/user-image1.jpg", "https://example.com/user-image2.jpg"]
}
```
...
- KoBERT는 일기 본문(`content`)만 분석하여 감정을 결정 (제목, 기분, 날씨, 활동 등은 참고하지 않음)
...
2. **AI 이미지 생성**: 일기 작성 내용(제목, 본문, 기분, 날씨, 활동)과 KoBERT 감정 분석 결과를 활용하여 NanoVana API로 그림일기 형태의 이미지 생성
   - 생성 완료 → 이미지 URL 획득
...
4. **AI 코멘트 생성**: 일기 내용(제목, 본문, 기분, 날씨, 활동)과 KoBERT 감정 분석 결과, 페르소나 스타일을 반영하여 Gemini API로 AI 코멘트 생성
5. **음식 추천 생성**: 일기 내용(제목, 본문, 기분, 날씨, 활동)과 KoBERT 감정 분석 결과를 반영하여 Gemini API로 음식 추천 생성 (DB에 저장)
...
- `weather`: 날씨 (선택)
- `activities`: 활동 목록 (선택, 문자열 배열)
- `imageUrl`: AI 생성 이미지 URL (선택, 기존 이미지 유지, 재생성하지 않음)
...
```json
{
  ...
  "data": {
    ...
    "images": ["https://example.com/user-image1.jpg"],
    "imageUrl": "https://example.com/ai-generated-image.jpg",
    "aiComment": "수정된 내용에 대한 새로운 AI 코멘트",
    "recommendedFood": {
      "name": "따뜻한 국밥",
      "reason": "몸을 따뜻하게 해주는 음식이 기분 전환에 도움이 될 수 있어요"
    },
...
2. **일기 데이터 저장**: 수정된 일기 데이터 전송 (제목, 본문, 기분, 날씨, 활동, AI 생성 이미지 URL, 사용자 업로드 이미지 URL 목록)
   ...
   - 이미지는 재생성하지 않음
3. **AI 코멘트 재생성**: 수정된 일기 내용(제목, 본문, 기분, 날씨, 활동)과 KoBERT 감정 분석 결과, 페르소나 스타일을 반영하여 Gemini API로 새로운 AI 코멘트 생성
4. **음식 추천 재생성**: 수정된 일기 내용(제목, 본문, 기분, 날씨, 활동)과 KoBERT 감정 분석 결과를 반영하여 Gemini API로 음식 추천 재생성 (DB에 업데이트)

**참고**: 
- 이미지는 재생성하지 않고 기존 이미지 유지
- KoBERT 결과는 AI 코멘트 재생성 및 음식 추천에 활용됨
...
**참고**: 일지 작성 개수 추이만 제공하며, 개인 정보는 포함하지 않습니다.
...
- `year`: 연도
- `month`: 월 (주간/월간 조회 시)
- `metrics`: 지표 선택 (여러 개 가능, 쉼표로 구분, 예: dau,wau,mau,newUsers,retentionRate)
  - `dau`: 일일 활성 사용자 수
  - `wau`: 주간 활성 사용자 수
  - `mau`: 월간 활성 사용자 수
  - `newUsers`: 신규 가입자 수
