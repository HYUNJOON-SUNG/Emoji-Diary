 ...
 * 
 * @description
 * 사용자 개인 정보 및 설정을 관리하는 메인 컴포넌트
 * - 유스케이스: 2.x 마이페이지 (사용자 명세서 참조)
 * - 모바일 웹 최적화된 UI 제공
 * 
 * @features
 * 1. 내 정보 확인:
 *    - 프로필 이미지(성별 기반), 이름, 이메일 표시
 * 2. AI 말투(페르소나) 설정:
 *    - PersonaSelectionModal 데이터(PERSONAS)를 활용하여 AI 답변 스타일 변경
 * 3. 비밀번호 변경:
 *    - 이메일 인증 -> 코드 확인 -> 새 비밀번호 설정을 통한 보안 프로세스
 * 4. 공지사항 조회:
 *    - AnnouncementPage와 연동하여 슬라이드 애니메이션으로 화면 전환
 * 5. 계정 관리:
 *    - 로그아웃 및 회원 탈퇴 기능
 * 
 * @props
 * - onBack: 뒤로가기 핸들러 (선택 사항)
 * - onUserUpdate: 사용자 정보 업데이트 콜백
 * - onAccountDeleted: 계정 삭제 완료 콜백
 * - onGoToSupport: 고객센터/도움말 이동 핸들러
 * - onModalStateChange: 모달 열림 상태 변경 콜백
 * - onLogout: 로그아웃 핸들러
 * 
 * ====================================================================================================
 */
